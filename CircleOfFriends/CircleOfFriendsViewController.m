...
@interface CircleOfFriendsViewController () <UITableViewDataSource,
                                             UITableViewDelegate> {
...
- (void)initTableViewHeaderView {
  ...
  UIView *headerView = [[UIView alloc]
      initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, HEADERHEIGHT)];
  headerView.backgroundColor = [UIColor whiteColor];
  UIImageView *imageView = [[UIImageView alloc]
      initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, HEADERHEIGHT)];
  imageView.image = [UIImage imageNamed:@"AlbumCover"];
  UIImageView *imageAvatar = [[UIImageView alloc]
      initWithFrame:CGRectMake(SCREEN_WIDTH - PADDING - selfAvatarWeight,
                               selfAvatarFromTop, selfAvatarWeight,
                               selfAvatarHeight)];
  ...
  userName.frame =
      CGRectMake(SCREEN_WIDTH - PADDING - selfAvatarWeight - selfUserNameWeight,
                 selfUserNameFromTop, selfUserNameWeight, selfUserNameHeight);
  ...
  [imageAvatar showPlaceHolder];
  self.contentTableView.tableHeaderView = headerView;
}
- (void)animation1 {
  //    POPSpringAnimation *rotationAnimation = [POPSpringAnimation
  ...
  //    [self.view.layer pop_addAnimation:rotationAnimation
  //    forKey:@"rotationAnim"];

  //    POPBasicAnimation *opacityAnimation = [POPBasicAnimation
  //    animationWithPropertyNamed:kPOPLayerOpacity];
  //    opacityAnimation.toValue = @(0.5);
  //    [self.view.layer pop_addAnimation:opacityAnimation
  //    forKey:@"opacityAnimation"];

  //    POPSpringAnimation *scaleAnimation = [POPSpringAnimation
  //    animationWithPropertyNamed:kPOPLayerScaleXY];
  //    scaleAnimation.toValue = [NSValue valueWithCGSize:CGSizeMake(0.5, 0.5)];
  //    scaleAnimation.springBounciness = 10.f;
  //    [self.view.layer pop_addAnimation:scaleAnimation forKey:@"scaleAnim"];

  //    POPDecayAnimation *decayAnimation = [POPDecayAnimation
  //    animationWithPropertyNamed:kPOPLayerPositionY];
  //    decayAnimation.velocity = @(1233);
  //    decayAnimation.deceleration = .823;
  //    [self.view.layer pop_addAnimation:decayAnimation forKey:@"aaa"];

  POPSpringAnimation *animation = [POPSpringAnimation animation];
  animation.property =
      [POPAnimatableProperty propertyWithName:kPOPLayerTranslationY];
  animation.fromValue = @300.0;
  animation.toValue = @0.0;
  animation.springBounciness = 10.0;
  animation.springSpeed = 12.0;
  [self.view.layer pop_addAnimation:animation forKey:@"pop"];
}

- (void)initTableView {
  CGRect frame =
      CGRectMake(0, -50, SCREEN_WIDTH, self.view.frame.size.height + 90);
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  ContentCell *cell =
      [ContentCell cellWithTableView:tableView identifier:@"circleFeeds"];
  cell.feedFrame = self.statusFrames[indexPath.row];
  return cell;
}

- (CGFloat)tableView:(UITableView *)tablXeView
